...
<body id="page-top">
          ...
          <div class="card shadow mb-4">
            ...
            <div class="card-body">
              <div class="row">
                ...
                <button class="btn btn-primary btn-lg m-2" onclick="location.href='panggilantrian_poli.php'" type="button">Lewati</button>
                
                <?php
                include "koneksii.php";
                $raw = "select * from audible where antrian_id_antrian = " . $id_antrian . " limit 1";
                $query = mysqli_query($conn, $raw);
                ...
                ?>
                <form action="selesaiantrian.php" id="selesai2" method="post">
                  <button class="btn btn-primary btn-lg m-2" id="selesai3" type="submit" <?= $disabledButton ?>>Sudah Datang</button> <input type="hidden" name="id_antrian" value="<?php echo $id_antrian; ?>">
  ...
  <script>
    $('#selesai2').on('submit', function() {
      $.ajax({
        url: "selesaiantrian.php", //the page containing php script
    ...
    $('#panggil-antrian').on('submit', function(e) {
      e.preventDefault()
      $.ajax({
        url: "panggilantrianpoliajax.php", //the page containing php script
        type: "POST", //request type
        data: $(this).serialize(),
        dataType: 'json',
        success: function(res) {
          if (res == 1) {
            $('#selesai3').prop('disabled', false)
            alert('Berhasil memanggil')
          } else {
            $('#selesai3').prop('disabled', true)
