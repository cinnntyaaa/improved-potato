...
$upd = $_GET['upd'];
$sql = "SELECT * from petugas where id_petugas='$upd'";
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-4 text-gray-800">Edit Akun Petugas</h1>

          <div class="row">
            <div class="col-lg-9">
              <form name='update' method='post' action='<?php $_SERVER['PHP_SELF']; ?>' enctype="multipart/form-data">
                <div class="form-group row">
                  <label for="InputUsername" class="col-sm-2 col-form-label">ID Petugas</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="id" id="id" value='<?php echo $hasil['id_petugas']; ?>' readonly>
                  </div>
                </div>
                <div class="form-group row">
                ...
                <div class="form-group row">
                  ...
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="nama" id="nama" value="<?= $hasil['nama_petugas']; ?>">
                  </div>
                </div>
                <div class="form-group row">
                  <label for="inputEmail3" class="col-sm-2 col-form-label">Poliklinik</label>
                  <div class="col-sm-10">
                    <select name="poliklinik" id="poliklinik" class="form-control">
                      ...
                      <?php if (mysqli_num_rows($query) > 0) { ?>
                        <option value="0" <?php if ($hasil['poliklinik'] == 0) {
                                            echo "selected";
                                          } ?>>admin</option>
                        <?php while ($row = mysqli_fetch_array($query)) {
                        ?>
                          <option value='<?php echo $row['id_poliklinik'] ?>' <?php if ($row['id_poliklinik'] == $hasil['poliklinik']) {
                                                                                echo "selected";
                                                                              } ?>><?php echo $row['nama_poliklinik'] ?></option>
                      <?php }
                      } ?>
                    </select>
                  </div>
                  <?PHP //echo $hasil['poliklinik'];
                  ?>
                </div>
                <div class="form-group row">
                  ...
                  <div class="col-sm-10">
                    <select name="level" id="level" class="form-control">
                      <?php
                      $selected = '';
                      if ($hasil['level'] == $hasil['level']) {
                        $selected = 'selected';
                      }
                      ?>
                      <option <?php echo $selected; ?>> <?php echo $hasil['level']; ?> </option>
                      ?>
                      <option>Admin</option>
                ...
                <div class="form-group row">
                  <div class="col-sm-10">
                    ...
                    <a href="akunpetugas_admin.php" name="keluar" class="btn btn-primary">Keluar</a>
                  </div>
                </div>
              </form>
              <?php
              if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $level = $_POST['level'];
                $ubah = $_POST['ubah'];
                $sql = "update petugas set nama_petugas='$nama_petugas', poliklinik='$poliklinik',
                        level='$level' where id_petugas='$id_petugas'";
                $query = mysqli_query($conn, $sql);
                if ($query) {
                  echo '<script>alert("Berhasil menyimpan data."); document.location="akunpetugas_admin.php?id_petugas=' . $id_petugas . '";</script>';
                } else {
