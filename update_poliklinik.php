...
$upd = $_GET['upd'];
$sql = "select * from poliklinik where id_poliklinik='$upd' ";
            ...
            <div class="col-lg-9">
              <form name='update' method='post' action='<?php $_SERVER['PHP_SELF']; ?>' enctype="multipart/form-data">
                <div class="form-group row">
                  <label for="InputUsername" class="col-sm-2 col-form-label">ID Poliklinik</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="id" id="id" value='<?php echo $hasil['id_poliklinik']; ?>' readonly>
                  </div>
                </div>
                <div class="form-group row">
                  <label for="inputEmail3" class="col-sm-2 col-form-label">Nama Poliklinik</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="nama" id="nama" value="<?= $hasil['nama_poliklinik']; ?>">
                  </div>
                </div>
                <div class="form-group row">
                  <label for="inputEmail3" class="col-sm-2 col-form-label">Kode</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="kode" id="kode" value="<?= $hasil['kode_poliklinik']; ?>">
                  </div>
                </div>
                <div class="form-group row">
                  <label for="inputEmail3" class="col-sm-2 col-form-label">Dokter</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="dokter" id="dokter" value="<?= $hasil['dokter']; ?>">
                  </div>
                </div>
                <div class="form-group row">
                  <div class="col-sm-10">
                    <button type="submit" name="ubah" class="btn btn-primary">Ubah</button>
                    <a href="poliklinik_admin.php" name="keluar" class="btn btn-primary">Keluar</a>
                  </div>
                </div>
              </form>
              <?php
              if ($_SERVER["REQUEST_METHOD"] == "POST") {
                $id_poliklinik = $_POST['id'];
                $nama_poliklinik = $_POST['nama'];
                $kode_poliklinik = $_POST['kode'];
                $dokter = $_POST['dokter'];
                $ubah = $_POST['ubah'];
                $sql = "update poliklinik set nama_poliklinik='$nama_poliklinik', kode_poliklinik='$kode_poliklinik',
                        dokter='$dokter' where id_poliklinik='$id_poliklinik'";
                $query = mysqli_query($conn, $sql);
                if ($query) {
                  echo '<script>alert("Berhasil menyimpan data."); document.location="poliklinik_admin.php?id_poliklinik=' . $id_poliklinik . '";</script>';
                } else {
